...
begin
    ...
    begin
        ...
        begin
            fTipZOffsets[x] := 0;
            TLogManager.Instance.Log('Remark 9W7.2.13.MöglicheFehlerquelle', true);
        end;
    end;
end;
...
begin
    ...
    begin
        TLogManager.Instance.Log('Remark 9W7.2.13.d1 - '+IntToStr(high(fInternMoveRacks)), true);
        ...
        TLogManager.Instance.Log('Remark 9W7.2.13.ha', true);
        TLogManager.Instance.Log(aCurrentPosition.RP[x].Rack.name, true);
        TLogManager.Instance.Log('Remark 9W7.2.13.hb', true);
        TLogManager.Instance.Log(aPreviousPosition.RP[x].Rack.name, true);
        // Sind die Racks identisch
        if (aCurrentPosition.RP[x].Rack.name <> aPreviousPosition.RP[x].Rack.name) then begin
        TLogManager.Instance.Log('Remark 9W7.2.13.hx', true);
            CONTINUE;
        end;
    ...
    TLogManager.Instance.Log('Remark 9W7.2.13.h2', true);
    EXIT(xAnyInternMove);
end;
    ...
    try
        ...
        begin
            ...
            begin
                ...
                TLogManager.Instance.Log('Remark 9W7.2.13.n4', true);
                FreeAndNil(xXYRoute);
                TLogManager.Instance.Log('Remark 9W7.2.13.n5', true);
            end;
        end;
        ...
        TLogManager.Instance.Log('Remark 9W7.2.13.o', true);
        fY.MoveY(xStep.Y, m_NO_EXEC, AT_MOVE_ABS, aYSpeed, aYRamp);
        TLogManager.Instance.Log('Remark 9W7.2.13.p', true);
        Execute();

        TLogManager.Instance.Log('Remark 9W7.2.13.q', true);
        if Assigned(fX) then
            fX.SetNextWaiting(false);
        TLogManager.Instance.Log('Remark 9W7.2.13.r', true);
    finally
        if xNewStep then
            FreeAndNil(xStep);
        TLogManager.Instance.Log('Remark 9W7.2.13.s', true);
    end;

    // nach jeder XY-Bewegung wird die Position gelöscht
    if Assigned(fOnSetPreviousPosition) then
        fOnSetPreviousPosition(nil);
    TLogManager.Instance.Log('Remark 9W7.2.13.t', true);
end;
...
begin
    ...
    TLogManager.Instance.Log('Remark 9W7.2.13.w', true);
    if Assigned(fOnSetPreviousPosition) then
        fOnSetPreviousPosition(aPreviousArmPosition);
    TLogManager.Instance.Log('Remark 9W7.2.13.x', true);
end;
