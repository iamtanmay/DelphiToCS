...
begin
            TLogManager.Instance.Log('Remark 9W7.2.13.1',true);
    FreeAndNil(fZTravelMotion);
            TLogManager.Instance.Log('Remark 9W7.2.13.2',true);
    FreeAndNil(fXYMotion);
            TLogManager.Instance.Log('Remark 9W7.2.13.3',true);
    inherited;
            TLogManager.Instance.Log('Remark 9W7.2.13.4',true);
end;
...
begin
    MoveXYWithOffset(aXYStep, 0, 0, aRP, aZStep, aOptions, aXSpeed, aXRamp, aYSpeed, aYRamp);
            TLogManager.Instance.Log('Remark 9W7.2.13.z',true);
end;
...
begin
    ...
    begin
        xIsInternMove := DetermineInternMove(xCurrentPosition, (not(xyoNeverUseRackZTravel in aOptions)));
            ...
            TLogManager.Instance.Log('Remark 9W7.2.13.j',true);
        fZTravelMotion.MoveToZTravelAllTips(0, 0);
            TLogManager.Instance.Log('Remark 9W7.2.13.k',true);

        fXYMotion.MoveXY(aXYStep, aXOffset, aYOffset, aOptions, xIsInternMove, aXSpeed, aXRamp,
            aYSpeed, aYRamp);
            TLogManager.Instance.Log('Remark 9W7.2.13.u',true);
    end;

    // Position speichern, auch wennn XY-Move nicht nötig war
            TLogManager.Instance.Log('Remark 9W7.2.13.v',true);
    fXYMotion.StoreXYPosition(xCurrentPosition);
            TLogManager.Instance.Log('Remark 9W7.2.13.y',true);
end;
