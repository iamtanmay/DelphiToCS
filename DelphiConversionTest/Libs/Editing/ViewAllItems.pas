{ --------------------------------------------------------------------------------------------------
  ...
  13.03.13 wl                               TN5960   alle Verweise auf ZADesignMain entfernt
  13.03.13 wl  FormShow                     TN5960   führt wieder ReadInfoHeight aus
  14.03.13 ts  AddChildAsLast               TN6092   neue Icons für startable methods
  14.03.13 wl                               TN5960   Erweiterungen, um im Runner, die richtigen Menüpunkte zu zeigen
  15.03.13 wl  ExpandMethodNode             TN5960   neu
  27.03.13 pp  pmnuRenameClick              TN5975   Umbenennen der Dateien in den Ordnern Methoden, LiquidHandling, SQLTerms
  17.04.13 wl  FileImport,FileExport        TN6106   neu
  24.05.13 wl  GetNamesAndAddChildren       TN6159   ReadAllNames wird mit Parameter ShowInTreeList aufgerufen
  30.08.13 wl                               TN6236   ReadOnly- und Hidden-Menu item entfernt
  -------------------------------------------------------------------------------------------------- }
...
type
    ...
    TfrmViewAllItems = class(TForm)
        ...
        procedure pmnuStartClick(Sender: TObject);
        procedure pmnuImportClick(Sender: TObject);
        procedure pmnuExportClick(Sender: TObject);
    private
        ...
        fFavouritesNode, fSpaceNode, fAllTopNode, fMethodTopNode, fSequenceTopNode, fWashProgTopNode,
            ...
            fSubstanceSetTopNode, fSQLTermTopNode, fDeviceTopNode, fDriverTopNode, fConnectionTopNode,
            fRackTopNode, fCarrierTopNode, fWorkspaceTopNode, fTipTypeTopNode, fImportFileDefTopNode,
            fTableImportDefTopNode, fVarImportDefTopNode, fDisplayComponentTopNode: TcxTreeListNode;
        fFormName: string;
        ...
        procedure StartSelectedItem();
        function GetDataFromNode(aNode: TcxTreeListNode; out oData: TViewItemDataRec): boolean;
        function CreateViewItemFromNode(aNode: TcxTreeListNode; out oViewItem: TViewItem): boolean;
        ...
        function ReadInfoHeight(const aFormName: string; aDefault: integer): integer;
        procedure WriteInfoHeight(const aFormName: string; aHeight: integer);
    public
        constructor Create(aOwner: TComponent; const aFormName: string;
            aDblClickEventType: TViewItemDblClickEventType); reintroduce;
        ...
        function AddChildAsLast(aNodeType: TViewItemType; const aNodeName: string; aSelect: boolean = true;
            aParent: TcxTreeListNode = nil): TcxTreeListNode;
        procedure DeleteSelectedNode();
        procedure ExpandMethodNode();
...
begin
    ...
    AddSingle(44567, '&Rename', '&Umbenennen');
...
begin
    ...
    result := TViewItemsWorkflow.Instance.GetTypeByNode(aNode);
    ...
    oIsFavourite := (result = ntFavFolder) or (aNode.Parent = fFavouritesNode) or
        (TViewItemsWorkflow.Instance.GetTypeByNode(aNode.Parent) = ntFavFolder);
end;
...
begin
    ...
    if self.CreateViewItemFromNode(cxTreeList1.FocusedNode, xViewItem) then
    begin
        TViewItemsWorkflow.Instance.OpenItem(xViewItem);
    end;
end;

procedure TfrmViewAllItems.StartSelectedItem();
var
...
begin
    ...
    if self.GetDataFromNode(cxTreeList1.FocusedNode, xData) then
        TViewItemsWorkflow.Instance.StartItem(xData);
end;
...
procedure TfrmViewAllItems.ExpandMethodNode;
begin
    fMethodTopNode.Expand(false);
end;
...
begin
    ...
    case (xNodeType) of
        ntMethod, ntSequence, ntWashProg, ntSQLTerm, ntLiquidPar, ntPowderPar, ntSubstance, ntSubstanceSet,
            ...
            ntImportFileDef:
            TViewItemsWorkflow.Instance.NewEditForm(xNodeType);
    end;
end;
...
var
    xImageIndex: integer;
begin
    xImageIndex := TViewItemsWorkflow.Instance.GetImageIndexByNode(aNode, aNode.Texts[0]);

    // ImageIndex und SelectedIndex sind (bisher) immer gleich
    aNode.ImageIndex := xImageIndex;
    aNode.SelectedIndex := xImageIndex;
end;

function TfrmViewAllItems.AddChildAsLast(aNodeType: TViewItemType; const aNodeName: string; aSelect: boolean;
    aParent: TcxTreeListNode): TcxTreeListNode;
var
...
begin
    ...
    result.StateIndex := TViewItemsWorkflow.Instance.GetIndexByType(aNodeType);
...
begin
    ...
    if not TViewItemsWorkflow.Instance.SaveAs(xSourceName, xNodeType) then
        EXIT;
end;
...
begin
    ...
    case (xNodeType) of
        ntMethod, ntSequence, ntWashProg, ntSQLTerm, ntLiquidPar, ntPowderPar, ntSubstance, ntSubstanceSet,
            ...
            ntImportFileDef:
            begin
                TViewItemsWorkflow.Instance.DeleteName(xNodeType, FRightSelected.Texts[0]);
            end;
    end;
end;
...
var
    i: integer;
    xIsTopNode, xIsFavourite, xSaveAllowed, xDeleteAllowed: boolean;
    xNodeType: TViewItemType;
    xViewItem: TViewItem;
begin
    xIsTopNode := false;
    ...
    xViewItem := TViewItemsWorkflow.Instance.CreateAnyViewItem(xNodeType, cxTreeList1.FocusedNode.Texts[0]);
    try
        if not xIsFavourite then
        begin
            pmnuNew.Visible := TViewItemsWorkflow.Instance.NewIsAllowed(xViewItem);
            pmnuImport.Visible := TViewItemsWorkflow.Instance.FileImportIsAllowed(xViewItem);
        end;

        if (xIsTopNode) then
            EXIT;

        if TViewItemsWorkflow.Instance.SearchPossible then
        begin
        ...
        if TViewItemsWorkflow.Instance.SaveAsIsAllowed(xViewItem) then
        begin
        ...
        begin
            pmnuStart.Visible := true;
            N_Start.Visible := true;
        end;

        if (not xIsFavourite) and TViewItemsWorkflow.Instance.DeleteIsAllowed(xViewItem) then
        begin
            N_Delete.Visible := true;
        ...
        if (xNodeType = ntFavFolder) or (xSaveAllowed and xDeleteAllowed) then
            pmnuRename.Visible := true;

        pmnuOpen.Visible := TViewItemsWorkflow.Instance.OpenIsAllowed(xViewItem);
        pmnuExport.Visible := TViewItemsWorkflow.Instance.FileExportIsAllowed(xViewItem);
        pmnuViewHierarchy.Visible := TViewItemsWorkflow.Instance.HierarchyIsAllowed(xViewItem);
    finally
        xViewItem.Free;
    end;

    if TViewItemsWorkflow.Instance.MethodOpenAndEditable and (xNodeType in [ntMethod, ntAction]) then
    begin
    ...
    if TViewItemsWorkflow.Instance.MethodOpenAndEditable and (xNodeType in [ntMethod]) then
    begin
        N_New.Visible := true;
    end;
...
begin
    self.cxTreeList1.Images := TViewItemsWorkflow.Instance.ActionImages.ActionImages16;
...
begin
    ...
    result := TViewItemsWorkflow.Instance.ReadAllNames(aNodeType, true);
...
function TfrmViewAllItems.CreateViewItemFromNode(aNode: TcxTreeListNode; out oViewItem: TViewItem): boolean;
var
    xData: TViewItemDataRec;
begin
    oViewItem := nil;
    result := self.GetDataFromNode(aNode, xData);
    if result then
        oViewItem := TViewItemsWorkflow.Instance.CreateAnyViewItem(xData.ItemType, xData.name);
end;
...
begin
    if self.CreateViewItemFromNode(cxTreeList1.FocusedNode, xViewItem) then
        TViewItemsWorkflow.Instance.AddItemToMethod(xViewItem);
end;
...
procedure TfrmViewAllItems.pmnuRenameClick(Sender: TObject);
var
    xName, xNameOld: string;
    xNodeType: TViewItemType;
    xIsTopNode, xIsFavorite: boolean;
begin
    xNodeType := GetTypeByNodeIntern(FRightSelected, xIsTopNode, xIsFavorite);
    if (xIsTopNode) then
        EXIT;
    ...
    begin
        // alten Namen erfassen und sichern
        xNameOld := FRightSelected.Texts[0];
        // neue Node speichern, Name wird in function abgefragt
        if not TViewItemsWorkflow.Instance.SaveAs(xNameOld, xNodeType) then
            EXIT;
        // alte Node entfernen
        TViewItemsWorkflow.Instance.DeleteWithoutAsking(xNodeType, xNameOld);
...
begin
    if self.CreateViewItemFromNode(cxTreeList1.FocusedNode, xViewItem) then
        TViewItemsWorkflow.Instance.AddItemToMethodParallel(xViewItem);
end;

procedure TfrmViewAllItems.FormActivate(Sender: TObject);
begin
    TViewItemsWorkflow.Instance.ShowInfo(cxTreeList1.FocusedNode, Memo1); // Refresh information
end;

function TfrmViewAllItems.ReadInfoHeight(const aFormName: string; aDefault: integer): integer;
var
...
procedure TfrmViewAllItems.WriteInfoHeight(const aFormName: string; aHeight: integer);
var
    xLocalIniFile: IConfigurationSet;
begin
    xLocalIniFile := TConfigurationFile.Create(TAppsettings.LocalIniFileName);
...
begin
    self.WriteInfoHeight(fFormName, Memo1.Height);
end;

procedure TfrmViewAllItems.FormDestroy(Sender: TObject);
begin
    TViewItemsWorkflow.Instance.RegisterAllItemsFormDestruction(self);
end;

procedure TfrmViewAllItems.FormShow(Sender: TObject);
begin
    Memo1.Height := self.ReadInfoHeight(fFormName, Memo1.Height);
end;
...
begin
    ...
    TViewItemsWorkflow.Instance.ShowInfo(cxTreeList1.FocusedNode, Memo1);
end;

procedure TfrmViewAllItems.cxTreeList1DblClick(Sender: TObject);
begin
    if (fDblClickEventType = dceOpen) then
        OpenSelectedItem()
    else
        StartSelectedItem();
end;
...
begin
    if (Sender = Source) and Assigned(cxTreeList1.DragNode) and
        not(TViewItemsWorkflow.Instance.GetTypeByNode(cxTreeList1.DragNode) in [ntFolder, ntFavFolder]) then
    begin

        xTargetNode := cxTreeList1.GetNodeAt(X, Y);

        if (TViewItemsWorkflow.Instance.GetTypeByNode(xTargetNode) in [ntFolder, ntFavFolder]) then
        ...
        begin
            ...
            if (TViewItemsWorkflow.Instance.GetTypeByNode(cxTreeList1.DragNode.Parent) <> ntFavFolder) and
                (cxTreeList1.DragNode.Parent <> self.fFavouritesNode) then
                fLastDragNodeTypes := [TViewItemsWorkflow.Instance.GetTypeByNode(cxTreeList1.DragNode)];
...
begin
    ...
    begin
        ...
        if (TViewItemsWorkflow.Instance.GetTypeByNode(xNode) = ntFavFolder) then
            Accept := true;
    end;
end;
...
begin
    if self.GetDataFromNode(cxTreeList1.FocusedNode, xData) then
        TViewItemsWorkflow.Instance.LoadHierarchy(xData.Name);
end;
...
begin
    TViewItemsWorkflow.Instance.SearchFor(FRightSelected.Texts[0]);
end;

procedure TfrmViewAllItems.pmnuStartClick(Sender: TObject);
begin
    StartSelectedItem();
end;

procedure TfrmViewAllItems.pmnuImportClick(Sender: TObject);
var
    xIsTopNode, xIsFavourite: boolean;
    xNodeType: TViewItemType;
begin
    ...
    TViewItemsWorkflow.Instance.FileImportEditForm(xNodeType);
end;

procedure TfrmViewAllItems.pmnuExportClick(Sender: TObject);
var
    xData: TViewItemDataRec;
begin
    if self.GetDataFromNode(cxTreeList1.FocusedNode, xData) then
        TViewItemsWorkflow.Instance.FileExport(xData);
end;
