{ --------------------------------------------------------------------------------------------------
  ...
  18.09.13 wl                               TN6045   SamDeviceName wird jetzt hier eingelesen
  -------------------------------------------------------------------------------------------------- }
...
begin
    ...
    xSamDeviceName := xIniAccess.ReadString('Device', 'Name');

    if (aTeachVol > 0) then
    begin
        fDllAdaptor := TWeighDbDllAdaptor_Teaching.Create(xSamDeviceName, xSubstanceID, aRunName, 0,
            aTeachVol, aTeachParam);
    end
    else
    begin
        fDllAdaptor := TWeighDbDllAdaptor_TargetWeight.Create(xSamDeviceName, xSubstanceID, aTargetWeight,
            aRunName, 0)
    end;
