{ --------------------------------------------------------------------------------------------------
  ...
  30.11.12 pp                                     TN6039   Neues Icon für UserProtection
  07.03.13 ts                                     TN6092   neue Icons für startable methods
...
const
    INT_MAX_ELEMENTS_PER_TYPE: integer = 100;
...
type
    ...
    private
        ...
        procedure SetHiddenMethodsAreShown(const Value: boolean);
    ...
    public
        ...
        procedure UpdateAttributes(const aMethodName: string; aAttributes: TMethodSetAttributes);
        function LoadLayout(const aLayoutName: string): boolean;
        ...
        property EditFunctions: TEditFunctions read fEditFunctions;
...
procedure TfrmEdMain.FormCreate(Sender: TObject);
var
...
begin
    ...
    FLinkAll := TZADesignLinkAll.Create;
    fEditFunctions := TEditFunctions.Create;
    TViewItemsWorkflow.OverviewManager := TViewItemOverviewManager.Create;
...
function TfrmEdMain.GetMainRunForm(): TfrmMainRun;
begin
    result := ffrmMainRunDevelopment.MainRun;
end;

function TfrmEdMain.GetCurrentEditModeForm: TMainCustomDevelopment;
var
...
function TfrmEdMain.GetCurrentEditMode: TAppEditMode;
begin
...
procedure TfrmEdMain.ShowAppMode(aMode: TAppEditMode);
begin
    case aMode of
        aemMethodDevelopment:
...
procedure TfrmEdMain.DefineCurrentEditor;
begin
...
function TfrmEdMain.AddMainTabSheet(aForm: TForm): TTabSheet;
var
...
procedure TfrmEdMain.DoOnGetStandardToolbarImageList(const aSender: TObject;
    out oImageList: TCustomImageList);
begin
    oImageList := self.ImagesButtons;
end;

procedure TfrmEdMain.DoOnGetStringLoader(const aSender: TObject; out oStringLoader: TStringLoader);
begin
    oStringLoader := fStringLoader;
end;

procedure TfrmEdMain.FormDestroy(Sender: TObject);
begin
...
procedure TfrmEdMain.FormShow(Sender: TObject);
var
    x: integer;
begin
    if fStarted then
    begin
        ...
        with TRunMainControls.Instance.MainFormControls do
        begin
            SetForm(self);
            SetPanelControls(self.actStartWithSelect, self.actStartLastStarted, self.actStartOpenMethod,
                self.actToolsFlush, self.actToolsInit, self.MainRun.lblJob, self.MainRun.PicPanel);
            SetMenus(mnuFile, nil, mnuTools, mnuUser, nil, nil, mnuMethod, nil);
            InitControls();
        end;

        actToolsFlush.Visible := fUseLiquids;

        if fDefaultMethod <> '' then
        begin
            self.actStartLastStarted.Visible := false;
        end;

        TEdExtern.Instance.SetOnSafeAppClose(self.PostClose);

        TRunMainControls.Instance.MainFormControls.SetRunControls(frmEdMain.MainRun.lblLayoutName,
            frmEdMain.MainRun.edLayoutName, frmEdMain.MainRun.lblMethod, frmEdMain.MainRun.edMethod,
            frmEdMain.MainRun.edSimulationCaption);

        TRunMainControls.Instance.MainPanel := frmEdMain.MainRun.pnlMain;
        TEdExtern.Instance.SetAnimationProperties(self.MainRun.ImageList1, self.MainRun.AnimateImage);
...
procedure TfrmEdMain.FormActivate(Sender: TObject);
begin
    RefreshButtons();
    Repaint;
end;

procedure TfrmEdMain.PostClose(aSender: TObject);
begin
    // Post a close message -> this will result in the Application Main Thread calling MainForm.Close
    PostMessage(self.Handle, WM_CLOSE, 0, 0);
end;

procedure TfrmEdMain.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
...
procedure TfrmEdMain.ExternalClose(const aMaxWaitTime: cardinal);
begin
    gLogManager.Log
...
procedure TfrmEdMain.WMExternalClose(var vMsg: TMessage);
begin
    self.ExternalClose(vMsg.WParam);
end;

function TfrmEdMain.CloseAllPages(): boolean;
begin
...
procedure TfrmEdMain.CloseLayout(const aLayoutName: string);
begin
    TZADesignLayoutManager.Instance.UnLoadFromMemory(aLayoutName)
end;

procedure TfrmEdMain.CloseAllLayouts();
begin
    TZADesignLayoutManager.Instance.UnloadAllFromMemory();
end;

// --------------------------------------------------------------------------------------------------
procedure TfrmEdMain.FormClose(Sender: TObject; var Action: TCloseAction);
// --------------------------------------------------------------------------------------------------
begin
    try
...
procedure TfrmEdMain.DockAndLoadForm(aForm: TDockableEditForm);
begin
    self.DockAndLoadForm(aForm, aemUnknown);
end;

procedure TfrmEdMain.DockAndLoadForm(aForm: TDockableEditForm; aEditMode: TAppEditMode);
begin
...
procedure TfrmEdMain.OpenTubeEditor(const aRackID: string);
var
...
procedure TfrmEdMain.pgctrlModesChange(Sender: TObject);
begin
    RefreshButtons(); // im Page-Control ändert sich die ActivePage
end;

procedure TfrmEdMain.pgctrlModesDrawTab(Control: TCustomTabControl; TabIndex: Integer; const Rect: TRect;
    Active: Boolean);
var
    xBitmap: TBitmap;
    xTrColor: TColor;
...
procedure TfrmEdMain.OpenPipetteParameter(const aName: string);
var
    xUsedTipType: string;
begin
    if not TLiqHDataAdaptor.Instance.ReadUsedTipType(aName, xUsedTipType) then
        EXIT;

    if TTipTypeDataAdaptor.TipNameIsRedi(xUsedTipType) then
        TViewItemsWorkflow.OpenEditForm(aName, ntPowderPar)
    else
        TViewItemsWorkflow.OpenEditForm(aName, ntLiquidPar)
end;

procedure TfrmEdMain.OpenExistingPipetteParameter(const aName: string);
begin
    if (aName = '') then
        EXIT;
    self.OpenPipetteParameter(aName);
end;

{ NEW actions }

procedure TfrmEdMain.actNewMethodExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntMethod);
end;

procedure TfrmEdMain.actNewWashProgramExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntWashProg);
end;

procedure TfrmEdMain.actNewSequenceExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntSequence);
end;

procedure TfrmEdMain.actNewLiquidParameterExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntLiquidPar);
end;

procedure TfrmEdMain.actNewPowderParameterExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntPowderPar);
end;

procedure TfrmEdMain.actNewDeviceExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntDevice);
end;

procedure TfrmEdMain.actNewDriverExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntDriver);
end;

procedure TfrmEdMain.actNewConnectionExecute(Sender: TObject);
begin
    TViewItemsWorkflow.NewEditForm(ntConnection);
end;

{ OPEN actions }

procedure TfrmEdMain.actOpenMethodExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntMethod);
end;

procedure TfrmEdMain.actOpenSequenceExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntSequence);
end;

procedure TfrmEdMain.actOpenWashProgramExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntWashProg);
end;

procedure TfrmEdMain.actOpenPowderParameterExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntPowderPar);
end;

procedure TfrmEdMain.actOpenLiquidParameterExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntLiquidPar);
end;

procedure TfrmEdMain.actOpenDeviceExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntDevice);
end;

procedure TfrmEdMain.actOpenDriverExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntDriver);
end;

procedure TfrmEdMain.actOpenConnectionExecute(Sender: TObject);
begin
    TViewItemsWorkflow.ChooseAndOpen(ntConnection);
end;

{ Other actions }

procedure TfrmEdMain.actImportIntoTableExecute(Sender: TObject);
var
...
procedure TfrmEdMain.actImportSequenceExecute(Sender: TObject);
var
    xNewName: string;
begin
    ...
    TViewItemsWorkflow.OverviewManager.InstanceAddChildAsFirstOrLast(ntSequence, xNewName);
    TViewItemsWorkflow.OpenEditForm(xNewName, ntSequence);
end;

procedure TfrmEdMain.actViewDefaultWorkspaceExecute(Sender: TObject);
begin
    TViewItemsWorkflow.OpenFirstItem(ntWorkspace);
end;

procedure TfrmEdMain.actViewDefaultTipTypeExecute(Sender: TObject);
begin
    TViewItemsWorkflow.OpenFirstItem(ntTipType);
end;

procedure TfrmEdMain.actFileSaveAsExecute(Sender: TObject);
var
...
begin
    ...
    TViewItemsWorkFlow.SaveAs(self.CurrentEditor.DataName, xNodeType);
end;

procedure TfrmEdMain.mnuUserChangePasswordClick(Sender: TObject);
var
    xUser: IUser;
begin
...
procedure TfrmEdMain.mnuUserLogonClick(Sender: TObject);
begin
...
procedure TfrmEdMain.actStartLastStartedExecute(Sender: TObject);
begin
...
procedure TfrmEdMain.actStartOpenMethodExecute(Sender: TObject);
begin
    if not Assigned(self.CurrentEditor) then
        EXIT;
...
procedure TfrmEdMain.actStartWithSelectExecute(Sender: TObject);
begin
...
procedure TfrmEdMain.actViewHierarchyExecute(Sender: TObject);
begin
    self.ShowAppMode(TAppEditMode.aemMethodDevelopment);
    ffrmMainMethodDevelopment.ShowViewHierarchy;
end;

procedure TfrmEdMain.actToolsFlushExecute(Sender: TObject);
begin
    if TEdExtern.Instance.ThreadIsRunning(false) then
        EXIT;
...
procedure TfrmEdMain.actToolsInitExecute(Sender: TObject);
begin
    if TEdExtern.Instance.ThreadIsRunning(false) then
        EXIT;
...
procedure TfrmEdMain.actUserChangePasswordExecute(Sender: TObject);
var
    xUser: IUser;
begin
    if TEdExtern.Instance.ThreadIsRunning(false) then
...
procedure TfrmEdMain.actUserLogonExecute(Sender: TObject);
begin
    if TEdExtern.Instance.ThreadIsRunning(false) then
        EXIT;
...
procedure TfrmEdMain.actHelpAboutExecute(Sender: TObject);
var
    xAboutBox: TAboutBox;
begin
...
procedure TfrmEdMain.SetFileSaveEnabled(const aEnabled: boolean);
begin
    actFileSave.Enabled := aEnabled;
    self.CurrentEditModeForm.SetFileSaveEnabled(aEnabled);
end;

procedure TfrmEdMain.SetFileSaveAllEnabled(const aEnabled: boolean);
begin
    actFileSaveAll.Enabled := aEnabled;
    self.CurrentEditModeForm.SetFileSaveAllEnabled(aEnabled);
end;

procedure TfrmEdMain.SetStartCurrentEnabled(const aMethodName: string; const aEnabled: boolean);
begin
    if aMethodName = '' then
    ...
    TRunMainControls.Instance.MainFormControls.EnableControlsByOpenMethod(aEnabled);
end;

procedure TfrmEdMain.RefreshButtons();
var
    // xCurrentEditMode: TAppEditMode;
...
begin
    ...
    begin
        ...
        actFileSaveAs.Enabled := self.CurrentEditor.SaveAsIsAllowed();

        if (self.CurrentEditor is TfrmMethodEditor) and
            ((self.CurrentEditor as TfrmMethodEditor).StartableFromGUI = msoStartable) then
        begin
...
procedure TfrmEdMain.RefreshSaveButtons();
begin
    // Grundeinstelleung
    SetFileSaveEnabled(false);
...
procedure TfrmEdMain.actFileSaveExecute(Sender: TObject);
begin
...
procedure TfrmEdMain.actFilePrintExecute(Sender: TObject);
begin
    if not Assigned(fCurrentEditor) then
        EXIT;
...
procedure TfrmEdMain.actFileSaveAllExecute(Sender: TObject);
begin
...
procedure TfrmEdMain.DoEditAction(aSender: TObject; aEditType: TEditActionType);
// Sender is a TAction
begin
...
procedure TfrmEdMain.DoRefreshButtons(aSender: TObject);
begin
    RefreshButtons;
end;

procedure TfrmEdMain.actEditUndoExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaUndo);
end;

procedure TfrmEdMain.actEditRedoExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaRedo);
end;

procedure TfrmEdMain.actEditCutExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaCut);
end;

procedure TfrmEdMain.actEditCopyExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaCopy);
end;

procedure TfrmEdMain.actEditPasteExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaPaste);
end;

procedure TfrmEdMain.actEditSelectAllExecute(Sender: TObject);
begin
    DoEditAction(Sender, eaSelectAll);
end;

procedure TfrmEdMain.actCompleteMethodImportExecute(Sender: TObject);
begin
    TfrmCompleteMethodImport.ShowAndImport;
end;

function TfrmEdMain.LoadLayout(const aLayoutName: string): boolean;
var
...
procedure TfrmEdMain.AddReagent(aReagentName: string);
begin
...
procedure TfrmEdMain.AfterSaveStatusChanged(Sender: TObject);
begin
    self.RefreshSaveButtons(); // wenn sich in dem Editor-Fenster selbst etwas ändert
end;

procedure TfrmEdMain.LoadLanguage(aApplObject: TScrollingWinControl);
var
    x: integer;
...
procedure TfrmEdMain.CloseDockClient(aForm: TForm; aEditMode: TAppEditMode);
begin
    if not Assigned(aForm) then
        EXIT;
...
function TfrmEdMain.FindDockClient(aCaption: string; aActivate: boolean; out oEditMode: TAppEditMode): TForm;
begin
...
procedure TfrmEdMain.SearchFor(const aSearchText: string);
begin
    self.ShowAppMode(TAppEditMode.aemMethodDevelopment);
    self.ffrmMainMethodDevelopment.SearchFor(aSearchText);
end;

procedure TfrmEdMain.UpdateAttributes(const aMethodName: string; aAttributes: TMethodSetAttributes);
begin
    ffrmMainMethodDevelopment.UpdateAttributes(aMethodName, aAttributes);
end;

{ Repository }

procedure TfrmEdMain.ExecuteDLLMenu(aSender: TObject);
var
    x: integer;
begin
...
procedure TfrmEdMain.CreateDllMenuItems();
var
    xNewItem: TMenuItem;
    x: integer;
begin
...
procedure TfrmEdMain.UserSetAccess;
var
    xUser: IUser;
begin
    xUser := gCommonDll.CurrentUser;
...
procedure TfrmEdMain.UpdateTV(aUpdateModes: TViewItemTypes);
begin
    TViewItemsWorkflow.OverviewManager.InstanceUpdateTV(aUpdateModes);
end;

procedure TfrmEdMain.SetHiddenMethodsAreShown(const Value: boolean);
begin
    fHiddenMethodsAreShown := Value;
    TViewItemsWorkflow.OverviewManager.InstanceUpdateTV([ntMethod]);
end;

procedure TfrmEdMain.actViewSearchExecute(Sender: TObject);
begin
    self.ShowAppMode(TAppEditMode.aemMethodDevelopment);
    self.ffrmMainMethodDevelopment.ShowViewSearch;
end;

procedure TfrmEdMain.actFindInMethodsExecute(Sender: TObject);
const
    cMaxSearchItems = 8;
var
...
procedure TfrmEdMain.actViewCompilerMessagesExecute(Sender: TObject);
begin
    self.ShowAppMode(TAppEditMode.aemMethodDevelopment);
    self.ffrmMainMethodDevelopment.ShowViewCompilerMessages;
end;

procedure TfrmEdMain.actViewConnectionsExecute(Sender: TObject);
var
...
procedure TfrmEdMain.CMDialogKey(var msg: TCMDialogKey);
var
    Control: TWinControl;
begin
    with Msg do
