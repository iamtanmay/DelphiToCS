{ --------------------------------------------------------------------------------------------------
  ...
  24.04.13 wl  WashDirtyTips                    TN6140   neu: WashAllDirtyTips nur für einen Arm
  15.08.13 wl  FindArmToUse                     TN6217   --> WeighWizardHandling, BasicPipetteRunStepBatcher
  15.08.13 wl                                   TN6223   uses geändert
  20.08.13 wl                                   TN6231   an Änderungen in ToolHandling angepasst
  WashHandling:
  21.08.13 wl                                   TN6231   unit umbenannt
  22.08.13 wl  InitializeSyringes,...           TN6231   --> DeviceInitHandling
...
type
    TWashHandling = record
    private
        class procedure PrepareWash(aPipDevice: IPipDevice; aPeripump: IPeriPumpDevice;
            out oWashVolCh1, oWashVolCh2: TArray<double>; out oWashRetrSpeed: integer;
            out oWashMethodName: string; out oDryAfterWash: TDryAfterWashType;
            out oUsePeripump: boolean); static;
        class function GetDirtyTipsNeedingAutoWash(aPipDevice: IPipDevice): TIPMAP; static;
    public
        class procedure VerifyTipNumber(aPipDevice: IPipDevice; aArmTipIndex: integer); static;
        class function GetCurrentBatchID(): integer; static;
        class procedure ExecuteWash(aUsedArm: IArmDevice; aRequestedTips: TIPMAP); static;
        class procedure WashAllDirtyTips(); static;
        class procedure WashDirtyTips(aUsedArm: IArmDevice); static;
        class procedure Wash(aUsedArm: IArmDevice; const aVolume, aVolume2: TArray<double>; aCycles: integer;
            ...
            const aPumpIndices: TArray<integer>); static;
        class procedure SwitchDiluentAndWash(aUsedArm: IArmDevice; aTips: TIPMAP; aDilIndex: integer); static;
    end;
...
class procedure TWashHandling.Wash(aUsedArm: IArmDevice; const aVolume, aVolume2: TArray<double>;
    aCycles: integer; aUsePeripump: boolean; aWashRetractSpeed: integer; const aWashMethodName: string;
    aDryAfterWash: TDryAfterWashType; aDropTipsBeforewashMethod: boolean;
    const aPumpIndices: TArray<integer>);
var
    i: integer;
...
class procedure TWashHandling.SwitchDiluentAndWash(aUsedArm: IArmDevice; aTips: TIPMAP; aDilIndex: integer);
var
    i: integer;
...
begin
    ...
    // Tools wegbringen
    TToolHandling.ArmBringBackTool(aUsedArm, false);

    for i := 0 to Length(xTipUsageArray) - 1 do
    begin
        xPipDevice.SetUseTips(xTipUsageArray[i].TipTypeName, xTipUsageArray[i].TipMap, false);
        Wash(aUsedArm, TArrayUtils.GetDefinedDoubleArray(gFlushVol, aUsedArm.PipDevice.TipCount),
...
class function TWashHandling.GetCurrentBatchID(): integer;
begin
    result := TThreadAPI.GetCurrentThreadID();
end;

// --------------------------------------------------------------------------------------------------
class procedure TWashHandling.PrepareWash(aPipDevice: IPipDevice; aPeripump: IPeriPumpDevice;
    out oWashVolCh1, oWashVolCh2: TArray<double>; out oWashRetrSpeed: integer; out oWashMethodName: string;
...
// --------------------------------------------------------------------------------------------------
class procedure TWashHandling.ExecuteWash(aUsedArm: IArmDevice; aRequestedTips: TIPMAP);
// --------------------------------------------------------------------------------------------------
var
    i, xWashRetrSpeed: integer;
    iMaxVol: extended;
    xWashVolCh1, xWashVolCh2: TArray<double>;
...
begin
    ...
    try
        ...
        Wash(aUsedArm, xWashVolCh1, xWashVolCh2, 1, xUsePeripump, xWashRetrSpeed, xWashMethodName,
            xDryAfterWash, xDropTipsBeforewashMethod,
            TLiquidChannels.GetDefaultChannel1Array(aUsedArm.PipDevice));
    end;
end;

class procedure TWashHandling.VerifyTipNumber(aPipDevice: IPipDevice; aArmTipIndex: integer);
begin
    ASSERT(aArmTipIndex < aPipDevice.TipCount);
end;

class function TWashHandling.GetDirtyTipsNeedingAutoWash(aPipDevice: IPipDevice): TIPMAP;
var
    x: integer;
begin
...
class procedure TWashHandling.WashDirtyTips(aUsedArm: IArmDevice);
var
...
begin
    ...
    xTipsNeedingWash := GetDirtyTipsNeedingAutoWash(aUsedArm.PipDevice);
    ExecuteWash(aUsedArm, xTipsNeedingWash);
end;

class procedure TWashHandling.WashAllDirtyTips();
var
    x: integer;
begin
    ...
    begin
        WashDirtyTips(gArmManager.Arms[x])
    end;
end;
