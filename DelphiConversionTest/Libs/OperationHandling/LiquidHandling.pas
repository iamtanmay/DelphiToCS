{ --------------------------------------------------------------------------------------------------
  ...
  21.08.13 wl  FlushAfterInit,Dry,PickAir   TN6249   --> LiquidHandlingLow
  11.09.13 wl                               TN6249   PickLiquid-Aufrufe an LiquidHandlingLow angepasst
  17.09.13 wl  WashAtWasteRack/-WashRack    TN6252.1 verwendet EmptyAllDilutors
  25.09.13 wl                               TN6259   Verbessertes Logging
  26.09.13 wl  LiquidWash                   TN6250   Die Vacuumpumpe wird nur noch angeschaltet, wenn auch Tips getrocknet werden
  30.09.13 wl  LiquidWash,...               TN6260   Cycles als neuer Parameter
  30.09.13 wl  WashAtWashRack               TN6260   Für jeden Cycle wird die Spritze neu gefüllt: Auch mehrere kleine Volumen möglich
  28.10.13 wl  CalcDilutorSteps             TN6287   Die Prüfung auf MaxMotorStep darf nicht stattfinden, wenn MaxMotorStep=0
...
type
    ...
    private
        class function GetInWashOrWasteInfo(aPipDevice: IPipDevice; aRP: TArray<TXRackPosition>)
            : TArray<boolean>;
        ...
        class procedure WashAtWashRack(aUsedArm: IArmDevice; aPipDevice: IPipDevice;
            ...
            aCycles: integer; aUsePeriPump: boolean; aUseCorridor: boolean; aWashRetractSpeed: integer;
            aEmptyDilutors: boolean; const aPumpIndices: TIntArray);
        class procedure WashAtWasteRack(aUsedArm: IArmDevice; aPipDevice: IPipDevice;
            aZMotion: TZAxisPipMoveMotorMotionSystem; aTips: TIPMAP; const aVolume, aVolume2: TDoubleArray;
            aCycles: integer; aUsePeripump: boolean; aUseCorridor: boolean; aWashAtWaste: integer;
        ...
        class procedure EmptyDilutorsCh1(aPipDevice: IPipDevice; aTips: TIPMAP);
        class procedure ChangeValvePos1To4(aPipDevice: IPipDevice; aTips: TIPMAP; aValvePosConst: TValvePos);
        class procedure ExecuteLiquidWash(aUsedArm: IArmDevice; aTips, aDryTips: TIPMAP;
            const aVolume, aVolume2: TDoubleArray; aCycles: integer; aUsePeripump: boolean;
            aUseCorridor: boolean; aWashRetractSpeed: integer; aDryAfterWash: TDryAfterWashType;
            const aPumpIndices: TIntArray);
    public
        ...
        class procedure LiquidWash(aUsedArm: IArmDevice; const aVolume, aVolume2: TDoubleArray;
            aCycles: integer; aUsePeripump: boolean; aUseCorridor: boolean; aWashRetractSpeed: integer;
...
begin
    ...
    if Assigned(xSingleRack) and (xSingleRack.IsMultiTipRack()) then
    begin
        xv := 0;
...
begin
    ...
    TLiquidHandlingLow.PickLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices, aVol, aSpeed, V_SYR_TIP,
        aCh2WashVol, aCh2Speed, V_SYR_SYS, aVolStepsCh1, aVolStepsCh2, result,
        GetInWashOrWasteInfo(aPipDevice, aRP), true, true, aUselVolCorrect);
...
begin
    ...
    xTWay := TArrayUtils.GetDefinedArray<double>(-Abs(aRetractPos_mm), aPipDevice.TipCount);

    gLogManager.LogF('Aspiration from Z-Max with tracking of %f mm', [aRetractPos_mm], false);
    TLiquidHandlingLow.PickLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices, aVol, aSpeed, V_SYR_TIP,
        aCh2WashVol, aCh2Speed, V_SYR_SYS, aVolStepsCh1, aVolStepsCh2, xTWay,
        GetInWashOrWasteInfo(aPipDevice, aRP), true, true, aUseVolCorrect);
    gRunFlow.AppSleep(aDelay);
...
begin
    ...
    gLogManager.Log('Pick SystemLiq - Vol' + TLiquidHandlingLow.GetChannelVolumesBracketText(aVol,
        aCh1PumpIndices) + xCh2LogText + ' Speed' + TArrayUtils.ArrayToBracketText(aSpeed) + ' Delay[' +
        IntToStr(aDelay) + ']', true);
    ...
    TLiquidHandlingLow.PickLiquid(aPipDevice, aZMotion, aTips, aCh1PumpIndices, xVol1, xSpeed1, V_SYR_SYS,
        xVol2, xSpeed2, V_SYR_SYS, aVolStepsCh1, aVolStepsCh2, xTWay,
        TArrayUtils.GetFalseArray(aPipDevice.TipCount), true, cWaitAfterPickSys, aUseVolCorrect);
...
begin
    ...
    begin
        TLiquidHandlingLow.DispLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices, aVol, aSpeed, V_SYR_TIP,
            ...
            GetInWashOrWasteInfo(aPipDevice, aRP), true, true, aUseVolControl);
    end
    else
    begin
        TLiquidHandlingLow.DispLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices,
            ...
            aPipDevice.GetNullDoubleArray, aVolStepsCh2, aTWay, GetInWashOrWasteInfo(aPipDevice, aRP), true,
            true, aUseVolControl);
        TLiquidHandlingLow.DispLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices, aVol, aSpeed, V_SYR_TIP,
            aPipDevice.GetNullDoubleArray, aSpeedCh2, V_SYR_TIP, aVolStepsCh1, aPipDevice.GetNullDoubleArray,
            aTWay, GetInWashOrWasteInfo(aPipDevice, aRP), true, true, aUseVolControl);
    end;
...
begin
    ...
    if (not xUseTransportAir) then
        EXIT;
    gLogManager.Log('Aspirate Transport Air - Vol-' + TLiquidHandlingLow.GetChannelVolumesBracketText
        (aTransAirVol, aCh1PumpIndices) + ' Speed-' + TArrayUtils.ArrayToBracketText(aTransAirSpeed) +
        ' Delay-[' + IntToStr(aTransAirDelay) + ']', true);
    TLiquidHandlingLow.PickAir(aPipDevice, aZMotion, xSyrMap, aCh1PumpIndices, aTransAirVol, aTransAirSpeed,
        aTransAirDelay, aUseVolCorrect);
    if (gErrorManager.IsGlobalErr()) then
        EXIT;
...
begin
    ...
    TLiquidHandlingLow.PickAir(aPipDevice, aZMotion, aSyrMap, aCh1PumpIndices, aVol, aSpeed, aDelay,
        aUseVolControl); // Pick air
end;
    ...
    begin
        ...
        begin
            ...
            gLogManager.LogF('Aspirate Extra Gap Waste %d, Vol-%s, Speed-%s, Delay-%d',
                [xCount, TLiquidHandlingLow.GetChannelVolumesBracketText(xWasteVol, aCh1PumpIndices),
        ...
        begin
            ...
            gLogManager.LogF('Aspirate Extra Gap Air %d, Vol-%s, Speed-%s, Delay-%d',
                [xCount, TLiquidHandlingLow.GetChannelVolumesBracketText(xAirVol, aCh1PumpIndices),
...
begin
    ...
    xMixVol := TArrayUtils.GetDefinedArray<double>(vMix.Vol, aPipDevice.TipCount);
    xMixDVol := TArrayUtils.GetNullDoubleArray(aPipDevice.TipCount);
    xMixASpeed := TArrayUtils.GetDefinedArray<double>(vMix.AspSpeed, aPipDevice.TipCount);
    xMixDSpeed := TArrayUtils.GetDefinedArray<double>(vMix.DispSpeed, aPipDevice.TipCount);
...
begin
    ...
    try
        ...
        begin
            TLiquidHandlingLow.EmptyAllDilutors(aUsedArm.PipDevice, aTips, edtDispAll, 0);
        end;

        for xCycle := 0 to aCycles - 1 do
...
begin
    ...
    begin
        ...
        begin
            ...
            for x := 0 to aPipDevice.TipCount - 1 do
            begin
                xWashAtWasteVol1[x] := aVolume[x] * aCycles * aWashAtWaste / 100;
                xWashAtWasteVol2[x] := aVolume2[x] * aCycles * aWashAtWaste / 100;
            end;
...
class procedure TLiquidHandling.LiquidWash(aUsedArm: IArmDevice; const aVolume, aVolume2: TDoubleArray;
    aCycles: integer; aUsePeripump: boolean; aUseCorridor: boolean; aWashRetractSpeed: integer;
    aDryAfterWash: TDryAfterWashType; const aPumpIndices: TIntArray);
var
    x: integer;
    xTips, xDryTips: TIPMAP;
    iDevPump: ISwitchDevice;
begin
    xTips := 0;
    ...
    if (aDryAfterWash = dawUseTipParameter) then
        xDryTips := xTips and aUsedArm.PipDevice.DryAfterFlushTips
    else if (aDryAfterWash = dawDoDryAfterWash) then
        xDryTips := xTips
    else
        xDryTips := 0;

    iDevPump := TLiquidHandlingLow.FindDrySwitchDevice(dsdVacuumPump, aUsedArm.PipDevice);
    if (xDryTips > 0) and Assigned(iDevPump) then
        iDevPump.SwitchOn(false);
    try
        // Waschen ausführen
        ExecuteLiquidWash(aUsedArm, xTips, xDryTips, aVolume, aVolume2, aCycles, aUsePeripump, aUseCorridor,
            aWashRetractSpeed, aDryAfterWash, aPumpIndices);
    finally
        if (xDryTips > 0) and Assigned(iDevPump) then
            iDevPump.SwitchOff(false);
    end;
end;

class procedure TLiquidHandling.ExecuteLiquidWash(aUsedArm: IArmDevice; aTips, aDryTips: TIPMAP;
    const aVolume, aVolume2: TDoubleArray; aCycles: integer; aUsePeripump: boolean; aUseCorridor: boolean;
    aWashRetractSpeed: integer; aDryAfterWash: TDryAfterWashType; const aPumpIndices: TIntArray);
var
    xWashAtWaste: integer;
    xPipDevice: IPipDevice;
    xZMotion: TZAxisPipMoveMotorMotionSystem;
    xZTravelMotion: TZAxisTravelMotionSystem;
...
begin
    ...
    try
        ...
        if (aVolume[0] = 999) then
            TLiquidHandling.AcetoneWash(aUsedArm, aTips, xZMotion);

        // Empty dilutors and wash at WASTE rack
        if (xUseWasteRack) then
            WashAtWasteRack(aUsedArm, xPipDevice, xZMotion, aTips, aVolume, aVolume2, aCycles, aUsePeripump,
                aUseCorridor, xWashAtWaste, aPumpIndices);

        // (Empty dilutors and) wash at WASH rack
        WashAtWashRack(aUsedArm, xPipDevice, xZMotion, aTips, aVolume, aVolume2, aCycles, aUsePeripump,
            aUseCorridor, aWashRetractSpeed, not xUseWasteRack, aPumpIndices);
        ...
        TLiquidHandlingLow.Dry(aUsedArm, aDryTips);

        // Tips-Status zurücksetzen
        gmResetWashFlags(aUsedArm.PipDevice, aTips);
    finally
        FreeAndNil(xZMotion);
    end;
end;

class procedure TLiquidHandling.ChangeValvePos1To4(aPipDevice: IPipDevice; aTips: TIPMAP;
    aValvePosConst: TValvePos);
var
    i: integer;
...
begin
    if (not gErrorManager.IsGlobalErr) then
        for i := 0 to aPipDevice.TipCount - 1 do
            ...
            begin
                xDilutorDev := aPipDevice.Tips[i].GetPipPump(0);
                if Assigned(xDilutorDev) then
                    xDilutorDev.TurnValve(aValvePosConst);
            end;
...
class procedure TLiquidHandling.EmptyDilutorsCh1(aPipDevice: IPipDevice; aTips: TIPMAP);
var
    xTip: integer;
    xDilutorDev: IPipPumpDevice;
begin
    ...
    for xTip := 0 to aPipDevice.TipCount - 1 do
    begin
        if not TTipMapUtils.TipSelected(aTips, xTip) then
            CONTINUE;

        xDilutorDev := aPipDevice.Tips[xTip].GetPipPump(0);
        if Assigned(xDilutorDev) then
...
begin
    ...
    try
        ...
        // Ventile drehen
        ChangeValvePos1To4(xPipDevice, iTips, V_SYR_TIP);
        EmptyDilutorsCh1(xPipDevice, iTips); // Rest herausblasen!
        for x := 1 to 2 do // 2 mal Ansaugen und Rest herausblasen
            TLiquidHandlingLow.FlushSyringe(xPipDevice, aZMotion, xQuenchRack,
                TArrayUtils.GetDefinedDoubleArray(500, xPipDevice.TipCount), xPipDevice.GetNullDoubleArray, 0,
                TLiquidChannels.GetDefaultChannel1Array(xPipDevice));
...
begin
    ...
    try
        ...
        begin
            ...
            begin
                result[x] := xPipPumpDevice.VolumeULToVolumeSteps(aVolArray[x] * xVolCorrFactor);

                // MaxMotorStep=0 bedeutet, dass bauformbedingt gar kein Maximum existiert
                if (xPipPumpDevice.MaxMotorStep > 0) then
                    result[x] := MinValue([result[x], xPipPumpDevice.MaxMotorStep]);
            end;
        end;
    finally
...
class function TLiquidHandling.GetInWashOrWasteInfo(aPipDevice: IPipDevice; aRP: TArray<TXRackPosition>)
    : TArray<boolean>;
var
    x: integer;
begin
    ...
    begin
        ...
        if Assigned(aRP[x].Rack) then
            result[x] := TRackDefinitions.RackIsWashOrWaste(aRP[x].Rack.name, aPipDevice.RackNamePrefix);
    end;
end;
...
begin
    ...
    xVolCorrCurves := TArrayUtils.GetDefinedArray<string>(aVolCorrCurve, aPipDevice.TipCount);
    ...
    TLiquidHandlingLow.PickLiquid(aPipDevice, xZMotion, aUsedTips, xCh1PumpIndices, aVolumes, aSpeeds,
        ...
        aPipDevice.GetNullDoubleArray(), xTWay, GetInWashOrWasteInfo(aPipDevice, xRP), aTurnValves,
        aExecute, true);
