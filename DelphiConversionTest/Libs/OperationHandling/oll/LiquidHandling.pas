...
type
    ...
    private
        ...
        class procedure PipDelay(aPipDevice: IPipDevice; aDelay: integer);
...
class function TLiquidHandling.FindDrySwitchDevice(aDeviceType: TDrySwitchDeviceType; aPipDevice: IPipDevice)
    : ISwitchDevice;
const
...
class procedure TLiquidHandling.PickAir(aPipDevice: IPipDevice; aZMotion: TZAxisPipMoveMotorMotionSystem;
...
begin
    ...
    gLogManager.Log('Pick air volume ' + TArrayUtils.ArrayToBracketText(aVol) + ', speed ' +
        TArrayUtils.ArrayToBracketText(aSpeed), false);
    TLiquidHandlingLow.PickLiquid(aPipDevice, aZMotion, aTipMap, aCh1PumpIndices, aVol, aSpeed, V_SYR_TIP,
        aPipDevice.GetNullDoubleArray, aPipDevice.GetNullDoubleArray, V_SYR_TIP, xAirSteps,
        aPipDevice.GetNullDoubleArray, xTWay, true, true, aUseVolCorrect); // Take Airgap
    PipDelay(aPipDevice, aDelay);
...
begin
    ...
    PickAir(aPipDevice, aZMotion, xSyrMap, aCh1PumpIndices, aTransAirVol, aTransAirSpeed, aTransAirDelay,
        aUseVolCorrect);
    if (gErrorManager.IsGlobalErr()) then
        EXIT;
...
begin
    ...
    PickAir(aPipDevice, aZMotion, aSyrMap, aCh1PumpIndices, aVol, aSpeed, aDelay, aUseVolControl); // Pick air
end;
...
class procedure TLiquidHandling.FlushAfterInit(aUsedArm: IArmDevice; aRack: TRack; aTips: TIPMAP);
var
    i: integer;
...
begin
    ...
    TLiquidHandlingLow.FlushSyringe(xPipDevice, xZMotion, aRack, xVol, xPipDevice.GetNullDoubleArray, 0,
        xCh1PumpIndices);
    ...
    begin
        xVol := TArrayUtils.GetDefinedDoubleArray(gAirVolAfterInit, xPipDevice.TipCount);
        xSpeed := TArrayUtils.GetDefinedDoubleArray(200, xPipDevice.TipCount);
...
class function TLiquidHandling.InitSyringes(aUsedArm: IArmDevice; aTips: TIPMAP; aInitID: TDateTime): TRack;
begin
    result := nil;
    ...
    result := TLiquidHandlingLow.MoveToWasteRack(aUsedArm, aTips);

    aUsedArm.PipDevice.InitSyringes(aInitID, aTips);
end;
...
class procedure TLiquidHandling.Dry(aUsedArm: IArmDevice; aTips: TIPMAP);
var
...
class procedure TLiquidHandling.PipDelay(aPipDevice: IPipDevice; aDelay: integer);
begin
    if (aDelay > 0) then
    begin
        // 14.07.10 pk removed because no Pump.WaitFor was called here: aPipDevice.ExecutePipPumps(); // wait for end of moves
