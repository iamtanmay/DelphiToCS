...
type
    ...
    public
        ...
        class procedure WashDirtyTips(aUsedArm: IArmDevice);
        class procedure WashCombinedArm(aUsedArm: IArmDevice);
        class procedure Wash(aUsedArm: IArmDevice; const aVolume, aVolume2: TDoubleArray;
        ...
        class procedure SwitchDiluentAndWash(aUsedArm: IArmDevice; aTips: TIPMAP; aDilIndex: integer);
    end;
...
begin
    ...
    begin
        iDevPump := TLiquidHandling.FindDrySwitchDevice(dsdVacuumPump, xPipDevice);
        if Assigned(iDevPump) then
            iDevPump.SwitchOn(false);
        try
            // ---------------------------------------- normales Waschen der Nadeln (in WASTE- und WASH-Rack)
            TLiquidHandling.DoLiquidWash(aUsedArm, xVolume, xVolume2, aUsePeripump, true, aWashRetractSpeed,
                aDryAfterWash, aPumpIndices); // mit Corridor-Move
...
class procedure TWashManager.SwitchDiluentAndWash(aUsedArm: IArmDevice; aTips: TIPMAP; aDilIndex: integer);
var
    i: integer;
...
begin
    ...
    if not gSysLiqManager.ChangeLiquid(xPipDevice, aTips, aDilIndex) then
        EXIT;

    xUsableTips := gSysLiqManager.GetCurrentSysLiquidTipMap(xPipDevice, aDilIndex);
    if (xUsableTips = 0) then
    begin
        gErrorManager.SetGlobalErr(ERR_USER, Format('System Liquid %d is not connected to the dilutors!',
            [aDilIndex + 1]));
        EXIT;
    end;
    ...
    gLogManager.LogF('Init syringes after changing diluent - Tipmap: %d', [xUsableTips], true);
    ...
    begin
        ...
        TWashManager.Wash(aUsedArm, TArrayUtils.GetDefinedDoubleArray(gFlushVol, aUsedArm.PipDevice.TipCount),
            TArrayUtils.GetDefinedDoubleArray(gFlushVol, aUsedArm.PipDevice.TipCount), false, 0, '',
...
begin
    ...
    try
        ...
        TWashManager.Wash(aUsedArm, xWashVolCh1, xWashVolCh2, xUsePeripump, xWashRetrSpeed, xWashMethodName,
...
class procedure TWashManager.WashDirtyTips(aUsedArm: IArmDevice);
var
