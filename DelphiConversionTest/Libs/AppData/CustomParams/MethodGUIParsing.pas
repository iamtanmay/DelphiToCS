{ --------------------------------------------------------------------------------------------------
  ...
  25.02.13 wl                                       TN6095   uses geändert
  11.04.13 wl  TStringKeyStringValueList            TN6045   von ListClasses hierher
  12.08.13 wl                                       TN6214   geändert: Array-Format bisher [a,b,c], jetzt A(a,b,c)
  ---------------------------------------------------------------------------------------------------------------------- }
...
type
    TStringKeyStringValueList = class(TKeyValueList<string, string>)
    public
...
{ TStringKeyStringValueList }

constructor TStringKeyStringValueList.Create(const aSortDuplicates: TKeyValueListDuplicates;
...
begin
    ...
    inherited Create(false, xComparer, aSortDuplicates);
end;
...
begin
    ...
    begin
        ...
        result := TArrayUtils.StringToDoubleArrayDef(TArrayFormat.GetValueText(aValue),
            TArrayFormat.Delimiter, aDefault, TFormatUtils.GetSettingsEnglishUS);
...
begin
    ...
    begin
        // das Ergebnis ist ein Array
        oPosArray := TArrayFormat.ArrayFormatToStringArray(aValue);
        EXIT(true);
    end
    else
    begin
...
begin
    ...
    begin
        // das Ergebnis ist ein Array
        result := TArrayFormat.ArrayFormatToStringArray(aValue);
        if (Length(result) < aNoOfPos) then
            raise Exception.Create('Array does not fit to number of positions: ' + aValue);
    end
    else
...
begin
    ...
    begin
        // das Ergebnis ist ein Array
        result := TArrayUtils.StringToIntArrayDef(TArrayFormat.GetValueText(aValue), TArrayFormat.Delimiter,
            aDefault);
        if (Length(result) < aNoOfPos) then
            raise Exception.Create('Array does not fit to number of positions: ' + aValue);
    end
...
begin
    ...
    begin
        // das Ergebnis ist ein Array
        result := TArrayUtils.StringToIntArray(TArrayFormat.GetValueText(aValue), TArrayFormat.Delimiter);
        if (Length(result) < aNoOfPos) then
            raise Exception.Create('Array does not fit to number of positions: ' + aValue);
