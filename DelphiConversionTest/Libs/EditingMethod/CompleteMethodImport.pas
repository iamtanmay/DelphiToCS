...
type
    ...
    public
        ...
        procedure ReadToMethodStepList(aImportDefs: TMethImportDefsDataAdaptor;
            aMethodStepList: TObjectList<TMethodStep>);
    end;
    ...
    private
        ...
        //
        procedure ImportListToMethod(aStepList: TObjectList<TMethodStep>;
...
var
    x: integer;
    xNextStepOptions: TArray<string>;
begin
...
begin
    ...
    try
        if xDA.MethodExistsIntern(xExistingMethodName, false) then
...
begin
    xLayoutDef := fImportDefs.ReadMethImportDefsByKey(TCompleteMethodImportFile.STR_IMPORTFILE_IDENT_LAYOUT);
...
var
    ...
    xStepList: TObjectList<TMethodStep>;
    xLayoutName: string;
begin
    xLayoutName := '';
    ...
    xStepList := TObjectList<TMethodStep>.Create;
    try
        // Layout lesen
...
procedure TCompleteMethodImport.ImportListToMethod(aStepList: TObjectList<TMethodStep>;
    const aMethodName, aLayoutName: string);
var
    xMethodDA: TMethodDataAdaptor;
    xMethodSDA: TMethodSettingsDataAdaptor;
    xRec: TMethodRec;
    xMethodRec: TMethodSettingsRec;
    x: integer;
begin
    xMethodSDA := TMethodSettingsDataAdaptor.Create;
    try
        xMethodRec := TMethodSettingsDataAdaptor.GetEmptyRec;
        xMethodRec.MethodName := aMethodName;
        xMethodRec.LayoutName := aLayoutName;
        xMethodSDA.WriteRec(xMethodRec);
    finally
        FreeAndNil(xMethodSDA);
    end;
    ...
    try
        ...
        begin
            ...
            xRec := aStepList.Items[x].AsMethodRec(aMethodName, x + 1);

            // Write record into method.db
            xMethodDA.WriteMethodRec(xRec, true);
        end;
    finally
